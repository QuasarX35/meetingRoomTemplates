...
function displayNextMessage() {
    ...
    if (availability.toLowerCase() === 'available') {
        availabilityText.style.color = 'rgb(17, 183, 17)';
    } else {
...
function addOrRemoveElementBasedOnOrientation() {
    // Select the hr element between the "title" and "details" elements
    var hrElement = document.querySelector("#next-event #title + hr");
...
// Call the function to add or remove element based on orientation
addOrRemoveElementBasedOnOrientation();

// Listen for orientation change events to reevaluate the need for the element
window.addEventListener("resize", addOrRemoveElementBasedOnOrientation);